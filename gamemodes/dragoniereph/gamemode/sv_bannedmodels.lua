...
function GM:SaveBannedModels()
	...
	if !file.Exists("dragoniereph/","DATA") then
		file.CreateDir("dragoniereph")
	end
	...
	file.Write("dragoniereph/bannedmodels.txt", txt)
end

function GM:LoadBannedModels() 
	local jason = file.ReadDataAndContent("dragoniereph/bannedmodels.txt")
	if jason then
		local tbl = {}
		...
		// dont touch this
		// use ph_bannedmodels_menu or edit data/dragoniereph/bannedmodels.txt
		self.BannedModels = {
